  <template>
     <el-card class="box-card">
       ...
       <el-form :model="pwdForm" :rules="pwdFormRules" ref="pwdFormRef" label-width="100px">
         ...
         <el-form-item>
           <el-button type="primary" @click="updataPwdFn">修改密码</el-button>
...
export default {
  ...
  data () {
    ...
    const rePwd = (rules, value, callback) => {
      if (this.pwdForm.re_pwd === this.pwdForm.new_pwd) {
        callback()
      } else {
  ...
  methods: {
    updataPwdFn () {
      this.$refs.pwdFormRef.validate(async valid => {
        if (valid) {
          const { data } = await updataPwdAPI(this.pwdForm)
